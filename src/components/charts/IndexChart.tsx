    ...
    if (orientation === 'vertical') {
        return (
            <>
                    ...
                    {data.map(({ label, value }, index) => (
                        ...
                        >
                            <div className="bg-linear-to-t from-red-500/60 to-red-600/30 flex justify-between items-center w-full h-full text-nowrap  py-1">
                                <span className='text-xs text-neutral-200 w-full text-right pr-0.5'>{value} kg</span>
                            </div>
                ...
                {title && <div className='text-sm text-neutral-200 text-right px-6 py-2 font-display'>
                    {title}
                </div>}
            </>
        )
