...
function Chart({ processedData, title, stagedCount = 0 }: ChartProps) {
  ...
  const getHeightPercent = useCallback((weight: number) => 
    ((weight / maxWeight) * 100) / 2,
    [maxWeight]
  );
      ...
      <div className='flex justify-stretch items-stretch h-full w-full'>
        <div className='flex flex-col-reverse justify-between items-end h-full border-r border-black text-right bg-transparent w-7 pb-6'>
          ...
          {yAxisLabels.map((value, index) => (
            <div key={index} className='w-full text-xs text-neutral-50 relative'>
        ...
        >
          ...
          <div className='absolute inset-x-0 top-0 bottom-6 flex flex-col-reverse justify-between pointer-events-none'>
            {Array.from({ length: 20 }).map((_, index) => (
              <div key={index} className='border-b border-neutral-50/40' style={gridLineStyle} />
            ))}
          </div>
            ...
            {processedData.map((setRepGroups, setIndex) => {
              ...
              setRepGroups.forEach(rg => {
                for (let i = 0; i < rg.reps; i++) {
                  expandedBars.push({ weight: rg.weight, barIndex: expandedBars.length });
                }
              });

              // Group expanded bars by weight for visual stacking
              const groupedByWeight = expandedBars.reduce((acc, bar) => {
                if (!acc[bar.weight]) acc[bar.weight] = [];
                acc[bar.weight].push(bar);
                return acc;
              }, {} as Record<number, Array<{ weight: number; barIndex: number }>>);
              ...
              return (
                <div key={setIndex} className="flex items-end gap-1 h-full relative">
                  {weightGroups.map(([weight, bars], weightIndex) => {
                    ...
                    return (
                      <div key={weight} className="flex flex-col justify-end items-center gap-1 h-full relative">
                        <span
